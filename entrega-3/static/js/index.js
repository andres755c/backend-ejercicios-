const rutaFetch = 'http://localhost:4545/api/productos/'
...
function prepareFront() {
    fetch('http://localhost:4545/api/productos/cat/')
        .then(resp => resp.json())
        .then(data => {
...
function addProduct(e) {
    ...
    const rutaFetchPut = `http://localhost:4545/api/carritos/${activeCart}/add/${idProducto}` // /:cid/add/:pid
    fetch (rutaFetchPut, {
        method: "PUT",
        headers: {
